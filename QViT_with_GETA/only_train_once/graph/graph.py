...
class Graph:
    ...
    def get_param_groups(self):
        ...
        # param_groups = self._dedup_param_groups(param_groups)
        return param_groups.values()

    def _dedup_param_groups(self, param_groups):
        """移除跨 param_group 的重复参数，避免优化器初始化时报重复参数错误。"""
        seen_ptrs = dict()
        deduped = dict()

        for gid, group in param_groups.items():
            keep_idx = []
            for i, param in enumerate(group.get("params", [])):
                ptr = param.data_ptr()
                p_name = (
                    group["p_names"][i]
                    if "p_names" in group and i < len(group["p_names"])
                    else f"{gid}_param_{i}"
                )
                if ptr in seen_ptrs:
                    prev = seen_ptrs[ptr]
                    print(
                        f"[WARN][Graph] 参数 {p_name} (group {gid}) 已在 group {prev['group']} 的 {prev['name']} 中注册，忽略该重复条目。"
                    )
                    continue

                seen_ptrs[ptr] = {"group": gid, "name": p_name}
                keep_idx.append(i)

            if len(keep_idx) != len(group.get("params", [])):
