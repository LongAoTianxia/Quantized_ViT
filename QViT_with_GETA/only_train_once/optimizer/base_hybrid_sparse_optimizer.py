...
class BaseHybridSparseOptimizer(BaseOptimizer):
    def __init__(
        ...
        # 在交给 PyTorch Optimizer 之前，移除跨 param_group 的重复参数
        params = self._dedup_param_groups(params)
    ...
    def _dedup_param_groups(self, param_groups):
        """遍历删除出现在多个 param_group 中的同一参数，避免优化器报重复注册错误。"""
        seen_ptrs = dict()  # data_ptr -> {'group': id, 'name': name}
        deduped_groups = list()

        for group in param_groups:
            # 非 dict 或不含 params 的保持原样
            ...
            for i, param in enumerate(group["params"]):
                ...
                p_name = (
                    group["p_names"][i]
                    if "p_names" in group and i < len(group["p_names"])
                    else f"param_{i}"
                )
                if ptr in seen_ptrs:
                    prev = seen_ptrs[ptr]
                    # print(
                    #     f"[WARN][Optimizer] 参数 {p_name} (group {group.get('id', 'unknown')}) "
                    #     f"已在 group {prev['group']} 的 {prev['name']} 中注册，忽略该重复条目。"
                    # )
                    continue

                seen_ptrs[ptr] = {"group": group.get("id", "unknown"), "name": p_name}
                keep_idx.append(i)

            # 同步裁剪相关列表
            if len(keep_idx) != len(group["params"]):
                for key in ["params", "p_names", "p_transform", "op_names", "node_ids"]:
                    if key in group:
                        group[key] = [group[key][i] for i in keep_idx]
    ...
    def compute_importance_scores(self, **kwargs):
        ...
        # 3. 组合 overall，并计算每个 param_group 的 group_cost
        rho = 0.5
        eps = 1e-8
        global_start_idx = 0
        ...
        for group in self.param_groups:
            if group["is_prunable"] and not group["is_auxiliary"]:
                ...
                # 3.3 计算该 param_group 的 group_cost
                total_group_size = 0
                for param, p_transform in zip(group["params"], group["p_transform"]):
                    if p_transform == TensorTransform.NO_PRUNE:
                        continue
                    param_t = tensor_transformation_param_group(param.data, p_transform, group)
                    param_t = param_t.view(group["num_groups"], -1)
                    total_group_size += param_t.size(1)
                group["group_cost"] = max(total_group_size, 1)

                overall = group["importance_scores"]["overall"]
                self.global_scores.append(overall)

        # 4. 全局归一化 gl_scale，跨 param_group 比较
        all_inv = []
        for group in self.param_groups:
            if group["is_prunable"] and not group["is_auxiliary"]:
                overall = group["importance_scores"]["overall"]
                ...
                # === 修复问题1：数值稳定性 ===
                # 强制 overall 为正且有下界，避免除零或负值
                overall_safe = torch.clamp(overall.abs(), min=eps)
                
                # cost_factor 显式转为与 overall 同 device/dtype 的 tensor
                cost_factor = torch.tensor(
                    group["group_cost"] ** rho, 
                    device=overall.device, 
                    dtype=overall.dtype
                )
                
                inv = cost_factor / overall_safe  # [num_groups]
                
                # clamp inv 避免极端权重
                inv = torch.clamp(inv, min=0.1, max=10.0)
        ...
        all_inv_cat = torch.cat(all_inv, dim=0)
        # 处理潜在的 nan/inf
        all_inv_cat = torch.nan_to_num(all_inv_cat, nan=1.0, posinf=10.0, neginf=0.1)
        global_mean = all_inv_cat.mean().clamp(min=eps)

        # 用全局均值归一化
        idx = 0
        for group in self.param_groups:
            if group["is_prunable"] and not group["is_auxiliary"]:
                num_g = group["num_groups"]
                group["gl_scale"] = all_inv_cat[idx : idx + num_g] / global_mean
                idx += num_g
