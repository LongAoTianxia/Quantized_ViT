...
class GETA(BaseHybridSparseOptimizer):
    ...
    def compute_gamma_d(self, param_group, active_redundant_idxes, bit_range):
        ...
        if torch.mean(flatten_clip).item() < 1e-8:
            ...
            if torch.isinf(cosine_similarity_clip) or torch.isnan(
                ...
                # 异常情况：cosine_similarity_clip 超出 [-1, 1]
                # 记录警告，便于后续排查
                self.logger.warning(
                    f"Step {self.num_steps}: cosine_similarity_clip={cosine_similarity_clip.item():.6f} "
                    f"(clip_norm={flatten_clip_norm.item():.2e}, grad_norm={flatten_grad_norm.item():.2e})"
                )
                # Clamp 到有效范围后重新计算
                clamped_cos = torch.clamp(cosine_similarity_clip, -1.0, 1.0)
    ...
    def step(self, loss=None, closure=None):
        ...
        if (
            ...
            if (
                ...
                self.max_bit_wt = max(
                    self.min_bit_wt, 6, self.max_bit_wt - self.bit_reduction
                )
                self.max_bit_act = max(
                    self.min_bit_act, 6, self.max_bit_act - self.bit_reduction
                )
                print(f"Current max_bit_wt: {self.max_bit_wt}")
                print(f"Current max_bit_act: {self.max_bit_act}")
    ...
    def load_state_dict(self, state_dict):
        """
        加载优化器状态，重点是恢复剪枝相关的元数据。
        
        注意：模型权重应该通过 model.load_state_dict() 单独加载，
        这里主要恢复 pruned_idxes、num_steps 等优化器状态。
        """
        import copy

        # 恢复非 param_groups 的属性（num_steps, pruned_group_indices 等）
        for attr_name in state_dict:
            if attr_name == "param_groups":
                continue
            setattr(self, attr_name, state_dict[attr_name])
        
        # 恢复 param_groups 中的元数据（不包括 params tensor）
        if "param_groups" in state_dict:
            prev_param_groups = state_dict["param_groups"]
            for param_group in self.param_groups:
                ...
                if prev_param_group is None:
                    print(f"[WARN] Param group {param_group['id']} not found in saved state.")
                    continue
                
                # 构建 prev_param_group 中的参数名到 p_transform 的映射
                prev_name_to_transform = {}
                if "p_names" in prev_param_group and "p_transform" in prev_param_group:
                    for name, transform in zip(prev_param_group["p_names"], prev_param_group["p_transform"]):
                        prev_name_to_transform[name] = transform
                
                # 根据当前 param_group 的 p_names 顺序，重建 p_transform
                if prev_name_to_transform and "p_names" in param_group:
                    new_p_transform = []
                    for p_name in param_group["p_names"]:
                        if p_name in prev_name_to_transform:
                            new_p_transform.append(prev_name_to_transform[p_name])
                        else:
                            # 如果找不到，使用默认的 NO_PRUNE（对于量化参数）或 BASIC
                            from only_train_once.transform import TensorTransform
                            if any(q in p_name for q in ["d_quant", "t_quant", "q_m"]):
                                new_p_transform.append(TensorTransform.NO_PRUNE)
                            else:
                                new_p_transform.append(TensorTransform.BASIC)
                    param_group["p_transform"] = new_p_transform
                
                # 复制其他元数据（除 params, p_names, p_transform 外）
                for key in prev_param_group:
                    if key in ("params", "p_names", "p_transform"):
                        continue
                    param_group[key] = copy.deepcopy(prev_param_group[key])

        # 重建 auxiliary_param_groups
        self.auxiliary_param_groups = {
            group["id"]: group
            for group in self.param_groups
            if group.get("is_auxiliary", False)
