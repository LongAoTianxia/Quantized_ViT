...
def main(args):
    ...
    # 量化参数设置：从第1轮后开始量化，量化步长为总训练轮数的3/5
    start_projection_step = 3 * steps_per_epoch
    projection_steps = ((args.epochs - 5) * 3 // 5) * steps_per_epoch
    ...
    # 剪枝设置
    start_pruning_step = quantization_end_step + 1 * steps_per_epoch  # 在量化位宽到达4-8bit后开始剪枝
    # 剪枝到训练结束前2轮停止，增加缓冲期
...
if __name__ == '__main__':
    ...
    # GETA 特定参数
    parser.add_argument('--target_group_sparsity', type=float, default=0.5,  # 目标组稀疏度
    ...
    parser.add_argument('--group_lasso_lambda', type=float, default=1e-6,
                        help='lambda for group lasso penalty')
    parser.add_argument('--group_lasso_warmup_epochs', type=int, default=20,
