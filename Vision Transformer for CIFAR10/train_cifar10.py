...
def main(args):
    ...
    warmup_epochs = 2  # 可根据实际情况设为10~20
    ...
    for sweep_idx, (desc, quantize_head, quantize_patch_embed, attn_blocks, mlp_blocks_) in enumerate(sweep_configs):
        ...
        for stage, (w_bit, in_bit, out_bit) in enumerate(bit_stages):
            ...
            lf = lambda x: ((1 + math.cos(x * math.pi / bit_epochs[stage])) / 2) * (
                        1 - args.lrf) + args.lrf  # Cosine学习率调度
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=10)  # 10轮    量化模型可能需要更多轮次(模型内具体修改)
    parser.add_argument('--batch-size', type=int, default=32)  # 适合CIFAR-10的batch size
    parser.add_argument('--lr', type=float, default=0.001)  # 0.001  量化模型更小的学习率(模型内具体修改)
    parser.add_argument('--lrf', type=float, default=0.01)  # 学习率衰减系数
    parser.add_argument('--weight-decay', type=float, default=0.05)     # 权重衰减
    parser.add_argument('--mixed-precision', type=bool, default=True)   # 是否使用混合精度训练
    ...
    parser.add_argument('--teacher-weights', type=str,
                        default="D:/python/PycharmProjects/vision_transformer_for_CIFAR10/weights/Weighs_Save/model-best-cifar10.pth",
                        help='teacher weights path (FP32)')

    # 是否冻结权重
    parser.add_argument('--freeze-layers', type=bool, default=True)  # 若不量化，可以冻结权重
