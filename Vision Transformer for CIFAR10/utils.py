...
def read_split_data(root: str, val_rate: float = 0.2):
    ...
    # CIFAR-10类别名称映射（可选，如果想要自定义显示名称）
    # --------------------------------------------------------------------
    '''
    cifar10_display_names = {
        'airplane': '飞机',
        'automobile': '汽车',
        'bird': '鸟类',
        'cat': '猫',
    ...
    display_indices = {}    # 如果需要中文名称或其他自定义名称，可以使用cifar10_class_names替换class_indices
    for idx, class_name in enumerate(flower_class):
        display_name = cifar10_display_names.get(class_name, class_name)
        display_indices[idx] = display_name
    # 保存
    json_str = json.dumps(display_indices, indent=4, ensure_ascii=False)
    '''
    # --------------------------------------------------------------------
    # 生成类别名称以及对应的数字索引
    class_indices = dict((k, v) for v, k in enumerate(flower_class))
